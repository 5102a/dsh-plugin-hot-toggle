 ...
 * carrier the shipped Web plugins use. The factory returns { name, apply,
 * inject }; the client runtime calls apply(ctx) when the bundle mounts.
 *
 * UI: registers the "启停管理" tab inside the official Plugins settings
 * section (settings.plugins.tab), lists every installed plugin with source /
 * type filters and recent-first ordering, and hot-toggles entries through the
 * host's same-origin HTTP API (/plugin-hot-toggle/api).
 */
